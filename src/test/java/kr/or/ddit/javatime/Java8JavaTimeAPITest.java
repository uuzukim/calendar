...
class Java8JavaTimeAPITest {
	...
	@Test
	void test() {
		//1970/01/01 00:00:00 기준시로부터 몇 ms가 지났느냐로 시간을 환산. - epoch time
		Date today = new Date();
		System.out.println(today);
		
		// ISO 에서 협정된 표준시 : GMT, UTC , immutable 객체 (바뀌지 않는 객체라는 뜻/ 수정불가)
		...
		ZonedDateTime currentZ = ZonedDateTime.now(ZoneId.of("Africa/Abidjan"));// 서울 
		...
		YearMonth beforeMonth = currentYearMonth.minusMonths(6);
