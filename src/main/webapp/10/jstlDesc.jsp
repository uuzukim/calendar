...
<c:forEach var="i" begin="2" end="9" step="1" varStatus="vs">
	...
	<c:forEach var="j" begin="1" end="9" step="1" varStatus="vsInner">
...
<%-- 			<c:remove var="attr" scope="session"/> --%>
		...
		3) 반복문 : forEach, forTokens
			for : 일반for(선언절;조건절;증감절), 향상된 for(블럭변수선언절:반복대상 컬렉션)
		...
		4) URL 재작성  :url
			<a href="${pageContext.request.contextPath }/member/memberInsert.do?param1=value1&param2=value2">회원가입</a>
			<c:url value="/member/memberInsert.do" var="inserturl">
				<c:param name="param1" value="1"></c:param>			
				<c:param name="param2" value="2"></c:param>			
			</c:url>
