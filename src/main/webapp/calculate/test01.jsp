...
<div style="border:ipx solid black">
   <h4>request content-type header : server side numarshalling 여부 </h4>
...
//       this is window
      ...
      let data = null; //데이터를 직렬화  문자열은 폼 내의 입력 필드의 이름(name)과 값(value) 쌍을 key=value 형식으로 구성한 후, 각 쌍을 &로 연결한 결과
      let headers = {};
      let dataType = $('input[name=acceptType]:checked').val() ?? "html";
      //parameter인건 알겠는데 왜 json이냐 파라미터이냐야 
      ...
      let settings = {
         url: url,
         method: method,
         data: data, // 얘는뭘 담아서 보내주는거야 ?????????? 
         headers : headers,   
         dataType: dataType, // request Accept:application/json, response Content-type:application/json 응답데이터를 json 으로 줘!!
